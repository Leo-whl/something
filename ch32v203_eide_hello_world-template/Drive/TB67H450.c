...
void PWM_INB(uint16_t Compare)//CCR1是B
{
    TIM_SetCompare1(TIM3, Compare);//设置CCR1的值  占空比 = compare/1000
}
void PWM_INA(uint16_t Compare)//CCR2是A
{
    TIM_SetCompare2(TIM3, Compare);//设置CCR2的值  占空比 = compare/1000
}
...
//四步法控制电机转动
void StepCtrl(uint8_t step)//步数 控制电机走多少步 1步1.8°
{
